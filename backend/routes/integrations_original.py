...
def get_platform_actions(platform: str):
    """
...
def create_integration(
    ...
    try:
        ...
        test_result = test_integration_connection(new_integration, integration_data.credentials)
...
def test_integration_connection(integration: Integration, credentials: dict) -> dict:
    """Test integration connection"""
    try:
        platform = integration.platform
        
        if platform == IntegrationType.SLACK:
            return test_slack_connection(credentials)
        elif platform == IntegrationType.TRELLO:
            return test_trello_connection(credentials)
        elif platform == IntegrationType.NOTION:
            return test_notion_connection(credentials)
        elif platform == IntegrationType.GOOGLE_SHEETS:
            return test_google_sheets_connection(credentials)
        else:
            return {
                "success": False,
